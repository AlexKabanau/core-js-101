...
function isLeapYear(date) {
  ...
  //   return false;
  // } if (new Date(date).getFullYear() % 4) {
  //   return true;
  // }
  // return false;
...
function timeSpanToString(startDate, endDate) {
  const x = endDate - startDate;
  return new Date(x).toISOString().slice(11, -1);
  // throw new Error('Not implemented');
}
...
function angleBetweenClockHands(date) {
  ...
  let tetta = Math.abs(0.5 * (60 * h - 11 * m));
  if (tetta > 180) {
    tetta = Math.abs(360 - tetta);
  }
  return (tetta * Math.PI) / 180;
}
